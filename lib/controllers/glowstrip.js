...
var blue = new DevopsdConnection({port: 8321}),
    ...
    shimmer = new Shimmer({
      ...
      frequencySec: -2,
      mapShimmerToLightstrip: function(ls, waveValue) {
        ls.hsv(colorToShimmer, 1, 0.3 + 0.7 * Math.abs(waveValue));
      }
    });
...
function tickShimmer() {
  ...
  if (isShimmerAsleep) {
    for (tickShimmerI; tickShimmerI<NUM_PIXELS-6; tickShimmerI++) {
